...
| `empirical_features_v1.yaml` | Deterministic empirical X-only transforms plus fold-local expression-discovery contract | Checkpoint 03B local validation pending |
...
Checkpoint 03A has its own `agronomic_features_v1.yaml`; do not mutate the frozen Checkpoint 02 config. Checkpoint 03B uses `empirical_features_v1.yaml` and awaits local validation. Model comparison is 03C afterward and should receive its own experiment config.
