...
| `build_empirical_features_v1.py` | Target-free temporal-geometry / relative-condition / sensor-similarity layer | Checkpoint 03B local run |
| `run_expression_discovery_03b.py` | Training-fold-only expression recurrence audit over frozen Checkpoint 02 folds | Checkpoint 03B local run |
| `run_checkpoint_03b.py` | One-command local runner: rebuild empirical layer, then run fold-local discovery audit | preferred local entry point |
...
Checkpoint 03 is open. 03A is closed. Run and review 03B locally before starting 03C and must reuse `reports/checkpoint_02/cv_folds.csv`.
