...
| `run_checkpoint_03c2.py` | Competition-only nested model-family benchmark with fold-safe tuning/discovery | Checkpoint 03C.2 closed |\n| `run_checkpoint_03c3.py` | Deterministic OOF review of three finalists and fixed equal-weight ensembles | Checkpoint 03C.3 closed |\n| `run_checkpoint_03c4.py` | Leakage-safe nested stacking with honest level-1 cross-fitting, dimensionality-reduction bases and meta learners | Checkpoint 03C.4 current |
...
Checkpoint 03 is open. 03A/03B/03C.1/03C.2/03C.3 are closed. 03C.4 is the leakage-safe nested stacking benchmark.
