...
## 2026-09-21 — Checkpoint 05 global-local mixture implemented

After freezing Checkpoint 04F, one explicit final modeling hypothesis was reopened: whether the
best global experts from Checkpoint 03 and the best local/transductive experts from Checkpoint
04 have complementary errors that can be exploited by a leakage-safe ensemble.

This is treated as a new Checkpoint 05 rather than another 04 substage because it combines the
two previously separate modeling lines.

Checkpoint 05 uses PLS C0, Ridge C3 with fold-local discovery, CatBoost C1, ExtraTrees C1,
Local04D and Graph04D as base experts. Every meta-training prediction is generated by repeated
cross-fitting inside the labels visible to the current pseudo-competition. Global expert
hyperparameters and any target-aware discovery are themselves fitted only inside the
corresponding cross-fit training subset.

The principal new model is a support-conditioned softmax mixture-of-experts. It learns
parcel-specific convex weights from nearest geographic/agronomic support, relative support,
same-state/same-municipality label availability and Local-vs-Graph disagreement. Simpler
convex, Ridge, ElasticNet, Huber, shallow ExtraTrees and shallow HistGradientBoosting stacks
remain as controls.

The run covers the frozen target-matched pseudo-competitions plus state-random and legacy
state/municipality stress protocols. CatBoost remains GPU-first with no automatic CPU fallback.
Split-level partial artifacts are written so the long workstation run can resume after
interruption.

A predeclared promotion gate prevents a post-hoc development minimum from replacing the
incumbent automatically. A Checkpoint 05 candidate can supersede Local04D only if the
leave-one-pseudo-split-out method-selection procedure improves both mean and pooled
target-matched RMSE relative to the fixed 04F rule.
